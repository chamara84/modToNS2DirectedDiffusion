...
LevelEntry * GradientFilter::matchLevelEntry( float latitude,float longitude)
{
  ...
  for (level_itr = level_hierachy_.begin(); level_itr != level_hierachy_.end(); ++level_itr){
    ...
    Tcl::instance().evalf("puts \"Node: %d latitude:%f,longitude %f level %d\"",((DiffusionRouting *)dr_)->getNodeId(),level_entry->latitude_,level_entry->longitude_,level_entry->level_);
  
    if (level_entry->latitude_==latitude && level_entry->longitude_==longitude ){
...
void GradientFilter::processNewMessage(Message *msg)
{
  ...
  NRSimpleAttribute<float> *srcLongitude = NULL;
  NRSimpleAttribute<float> *srcLatitude = NULL;
  ...
  case INTEREST:
      Tcl::instance().evalf("puts \"in interest\"");
    nrclass = NRClassAttr.find(msg->msg_attr_vec_);
    nrscope = NRScopeAttr.find(msg->msg_attr_vec_);
    srcLongitude =LongitudeAttr.find(msg->msg_attr_vec_);
    srcLatitude =LatitudeAttr.find(msg->msg_attr_vec_);
	...
	(nrclass->getOp() == NRAttribute::IS)){
        ...
        level = matchLevelEntry(srcLatitude->getVal(),srcLongitude->getVal());
        ...
        if (!level){
            if(msg->last_hop_ == LOCALHOST_ADDR)
            {
                ...
                levelentry->latitude_ = srcLatitude->getVal();
                levelentry->longitude_ = srcLongitude->getVal();
                levelentry->level_ = level_higher;
            ...
            level_higher = numHopSink->getVal();
            level->level_ = numHopSink->getVal();
        ...
        /*
        if (this->level_hierachy_ == -1){
            if(msg->last_hop_ == LOCALHOST_ADDR)
            {
                this->level_hierachy_ = 0; }
            else
            {
            this->level_hierachy_ = numHopSink->getVal();
            }
           }
            
        else if ((this->level_hierachy_ >= numHopSink->getVal()) && (msg->last_hop_!= LOCALHOST_ADDR)){
        this->level_hierachy_ = numHopSink->getVal();
                }*/
    level = matchLevelEntry(srcLatitude->getVal(),srcLongitude->getVal());
          if(!level){
    ...
    Tcl::instance().evalf("puts \"Received Interest from %d level %d at %d,hav energy %f\"",msg->last_hop_,level_higher,((DiffusionRouting *)dr_)->getNodeId(),((DiffusionRouting *)dr_)->returnNode()->energy_model()->energy());  //create attributes
      ...
      if (nrscope->getVal() == NRAttribute::GLOBAL_SCOPE){
          msg->msg_attr_vec_->push_back(HopsFrmSinkAttr.make(numHopSink->getOp(),level_higher+1));
        //msg->msg_attr_vec_ = CopyAttrs(&attrs);
        ClearAttrs(&attrs);
